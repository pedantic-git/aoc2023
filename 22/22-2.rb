...
class Stack < Set
  ...
  # Work out how many bricks would fall for each disintegration
  def chains
    safes = Set.new(safe)
    supporters = supports
    supporters.to_h do |k,v|
      safes === k ? [k, []] : [k, chain(k, supporters)]
    end
  end

  private

  def chain(k, supporters)
    (supporters[k] + supporters[k].flat_map { chain(_1, supporters) }).uniq
  end
end
